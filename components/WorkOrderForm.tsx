...
export default function WorkOrderForm({ mode = 'admin-create', orderId }: WorkOrderFormProps) {
    ...
    useEffect(() => {
        console.log("Preview state changed", {
            ...
            pendingFileType: pendingFile instanceof File ? "File" : pendingFile instanceof Blob ? "Blob" : "null",
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (mode === 'admin-create') {
            ...
            if (!orderId) { alert("Falta ID de Orden"); return; }
        ...
        if (result.success) {
            ...
            if (mode === 'admin-create') {
                setFormData({
                    ...
                    pagoCapitana: 0, pagoMarinero: 0, deposito: 0, precioAcordado: 0, horasAcordadas: 0
                } as any);
    ...
    const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>, gastoType: string) => {
        ...
        if (files && files[0]) {
            ...
            console.log("State setters called - React will batch and update in next render");
        } else {
    ...
    const startCamera = useCallback(async (gastoType: string) => { // Takes gastoType
        setCameraError(null);
        stopStream(); 
        ...
        try {
            ...
            rearCameraId = rearCamera ? rearCamera.deviceId : videoInputs[videoInputs.length - 1]?.deviceId;
    ...
    const confirmUpload = async () => {
        // #region agent log
        fetch('http://127.0.0.1:7242/ingest/d4115199-c88c-42bb-816f-d1f922c49910',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'WorkOrderForm.tsx:276',message:'confirmUpload entry',data:{hasPendingFile:!!pendingFile,orderId,currentGastoType,hasPreviewUrl:!!previewUrl},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
        // #endregion
        ...
        // #region agent log
        fetch('http://127.0.0.1:7242/ingest/d4115199-c88c-42bb-816f-d1f922c49910',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'WorkOrderForm.tsx:279',message:'setUploading true',data:{uploadingSet:true},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
        // #endregion
        
        try {
            // 1. Convert Blob (from camera) to File object if necessary
            const fileName = `receipt-${currentGastoType}-${Date.now()}.jpg`;
            const fileToUpload = pendingFile instanceof Blob 
            ...
            // #region agent log
            fetch('http://127.0.0.1:7242/ingest/d4115199-c88c-42bb-816f-d1f922c49910',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'WorkOrderForm.tsx:290',message:'before calculateTotal/Balance',data:{},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{});
            // #endregion
            const total = calculateTotal();
            const balance = calculateBalance();
            
            // #region agent log
            fetch('http://127.0.0.1:7242/ingest/d4115199-c88c-42bb-816f-d1f922c49910',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'WorkOrderForm.tsx:295',message:'after calculateTotal/Balance',data:{total,balance},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{});
            // #endregion
            ...
            // #region agent log
            fetch('http://127.0.0.1:7242/ingest/d4115199-c88c-42bb-816f-d1f922c49910',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'WorkOrderForm.tsx:297',message:'before updateWorkOrder',data:{orderId},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
            // #endregion
            const saveRes = await updateWorkOrder(orderId, submissionData);
            
            // #region agent log
            fetch('http://127.0.0.1:7242/ingest/d4115199-c88c-42bb-816f-d1f922c49910',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'WorkOrderForm.tsx:299',message:'after updateWorkOrder',data:{saveSuccess:saveRes.success,saveError:saveRes.error},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
            // #endregion
            ...
            if (!saveRes.success) {
                ...
                setUploading(false);
                // #region agent log
                fetch('http://127.0.0.1:7242/ingest/d4115199-c88c-42bb-816f-d1f922c49910',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'WorkOrderForm.tsx:306',message:'updateWorkOrder failed - state reset',data:{},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
                // #endregion
                return;
            }
            ...
            // #region agent log
            fetch('http://127.0.0.1:7242/ingest/d4115199-c88c-42bb-816f-d1f922c49910',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'WorkOrderForm.tsx:315',message:'before uploadReceipt',data:{orderId,gastoType:currentGastoType,hasFile:!!fileToUpload},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
            // #endregion
            const res = await uploadReceipt(formDataUpload);
            
            // #region agent log
            fetch('http://127.0.0.1:7242/ingest/d4115199-c88c-42bb-816f-d1f922c49910',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'WorkOrderForm.tsx:316',message:'after uploadReceipt',data:{uploadSuccess:res?.success,uploadError:res?.error,hasResult:!!res},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
            // #endregion
            ...
            if (res.success && res.data) {
                ...
                // Refetch work order data to get updated receipts from server
                // This ensures we have the latest data even if the app refreshes
                if (orderId) {
                    try {
                        const refreshedOrder = await getWorkOrder(orderId);
                        ...
                        console.error("Error refreshing receipts:", refreshError);
                        // Fallback: add the new receipt to state if refetch fails
            ...
            // #region agent log
            fetch('http://127.0.0.1:7242/ingest/d4115199-c88c-42bb-816f-d1f922c49910',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'WorkOrderForm.tsx:335',message:'confirmUpload exception caught',data:{errorMessage:err?.message,errorName:err?.name,errorStack:err?.stack?.substring(0,200)},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
            // #endregion
            console.error("Error in confirmUpload:", err);
            alert("Error inesperado: " + (err?.message || "Por favor intente nuevamente"));
            ...
            setUploading(false);
            // #region agent log
            fetch('http://127.0.0.1:7242/ingest/d4115199-c88c-42bb-816f-d1f922c49910',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'WorkOrderForm.tsx:343',message:'exception handler - state reset',data:{},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
            // #endregion
        }
    };
    ...
    if (loading && !formData.nombre) return <div className="p-8 text-center">Cargando...</div>;
    ...
    return (
        <div className="bg-white rounded shadow-sm max-w-lg mx-auto overflow-hidden">
            ...
            <div className="space-y-3">
                <div>
                    <label className="block text-sm font-bold text-gray-700">Orden #</label>
                    <input value={orderId ? orderId.toString() : "Generada al guardar"} disabled className="w-full border p-3 rounded bg-gray-200 text-lg font-mono text-gray-600 cursor-not-allowed" />
                </div>
            ...
            <div>
                ...
                {isCameraActive && (
                    <div className="bg-gray-800 p-3 rounded-lg mb-4 text-center">
                        <p className="text-white mb-2">Captura de Recibo: **{currentGastoType}**</p>
                        {cameraError && <p className="text-sm text-red-500 mb-2">Error de Cámara: {cameraError}</p>}
                        <video 
                            ref={videoRef} 
                ...
                <div className="space-y-4">
                    {/* Combustible */}
                    <div>
                        <label className="block text-sm font-bold text-gray-700 mb-1">Combustible</label>
                        <div className="flex gap-2 items-center">
                            <input type="number" name="combustibleCost" value={formData.combustibleCost} onChange={handleChange} disabled={!canEdit('combustibleCost')} className="flex-1 border p-3 rounded font-mono text-lg" />
                            {isCaptain && orderId && !isCameraActive && (
                                <CameraButtons gastoType="combustible" startCamera={startCamera} handleFileSelect={handleFileSelect} uploading={uploading} />
                            )}
                        </div>
                        {isCaptain && getReceiptsByGasto("combustible").length > 0 && (
                            <div className="mt-2 flex flex-wrap gap-2">
                                {getReceiptsByGasto("combustible").map((r: any, i: number) => (
                                    <a key={i} href={r.url} target="_blank" rel="noopener noreferrer" className="block w-16 h-16 bg-gray-300 rounded overflow-hidden border hover:border-blue-500">
                    ...
                    {/* Hielo */}
                    <div>
                        <label className="block text-sm font-bold text-gray-700 mb-1">Hielo</label>
                        <div className="flex gap-2 items-center">
                            <input type="number" name="hieloCost" value={formData.hieloCost} onChange={handleChange} disabled={!canEdit('hieloCost')} className="flex-1 border p-3 rounded font-mono text-lg" />
                            {isCaptain && orderId && !isCameraActive && (
                                <CameraButtons gastoType="hielo" startCamera={startCamera} handleFileSelect={handleFileSelect} uploading={uploading} />
                            )}
                        </div>
                    ...
                    {/* Agua/Bebidas */}
                    <div>
                        <label className="block text-sm font-bold text-gray-700 mb-1">Agua/Bebidas</label>
                        <div className="flex gap-2 items-center">
                            <input type="number" name="aguaBebidasCost" value={formData.aguaBebidasCost} onChange={handleChange} disabled={!canEdit('aguaBebidasCost')} className="flex-1 border p-3 rounded font-mono text-lg" />
                            {isCaptain && orderId && !isCameraActive && (
                                <CameraButtons gastoType="aguaBebidas" startCamera={startCamera} handleFileSelect={handleFileSelect} uploading={uploading} />
                            )}
                        </div>
                        {isCaptain && getReceiptsByGasto("aguaBebidas").length > 0 && (
                            <div className="mt-2 flex flex-wrap gap-2">
                                {getReceiptsByGasto("aguaBebidas").map((r: any, i: number) => (
                                    <a key={i} href={r.url} target="_blank" rel="noopener noreferrer" className="block w-16 h-16 bg-gray-300 rounded overflow-hidden border hover:border-blue-500">
                                        <img src={r.url} alt="Receipt" className="w-full h-full object-cover" />
                                    </a>
                                ))}
                    ...
                    {/* Gasto Varios */}
                    <div>
                        <label className="block text-sm font-bold text-gray-700 mb-1">Gasto Varios</label>
                        <div className="flex gap-2 items-center">
                            <input type="number" name="gastoVariosCost" value={formData.gastoVariosCost} onChange={handleChange} disabled={!canEdit('gastoVariosCost')} className="flex-1 border p-3 rounded font-mono text-lg" />
                            {isCaptain && orderId && !isCameraActive && (
                                <CameraButtons gastoType="gastoVarios" startCamera={startCamera} handleFileSelect={handleFileSelect} uploading={uploading} />
                            )}
                        </div>
                        {isCaptain && getReceiptsByGasto("gastoVarios").length > 0 && (
                            <div className="mt-2 flex flex-wrap gap-2">
                                {getReceiptsByGasto("gastoVarios").map((r: any, i: number) => (
                                    <a key={i} href={r.url} target="_blank" rel="noopener noreferrer" className="block w-16 h-16 bg-gray-300 rounded overflow-hidden border hover:border-blue-500">
                                        <img src={r.url} alt="Receipt" className="w-full h-full object-cover" />
                                    </a>
                                ))}
            ...
            <div className="mt-2">
                ...
                <textarea name="detallesNotas" value={formData.detallesNotas} onChange={handleChange} disabled={!canEdit('detallesNotas')} rows={4} className="w-full border p-3 rounded" placeholder="Agregar notas aquí..." />
            </div>
            ...
            {mode === 'admin-edit' && receipts.length > 0 && (
                 <div className="mt-4 p-4 bg-gray-100 rounded border border-gray-300">
                        <h3 className="font-bold text-sm mb-4 uppercase text-gray-600">Comprobantes por Gasto</h3>
                            ...
                            return gastoTypes.map(({ key, label }) => {
                                ...
                                return (
                                    <div key={key} className="mb-4 last:mb-0">
                                        <h4 className="font-semibold text-sm text-gray-700 mb-2">{label}</h4>
                                        <div className="grid grid-cols-3 gap-2">
                                            {gastoReceipts.map((r: any, i: number) => (
                                                ...
                                                >
                                                    <img src={r.url} alt={`${label} Receipt ${i + 1}`} className="w-full h-full object-cover" />
                                                </button>
                                            ))}
                                        </div>
                                    </div>
                        ...
                        {(() => {
                            const ungroupedReceipts = receipts.filter((r: any) => !r.gastoType || r.gastoType === null);
                            if (ungroupedReceipts.length === 0) return null;
                            
                            return (
                                <div className="mt-4 pt-4 border-t border-gray-300">
                                    <h4 className="font-semibold text-sm text-gray-700 mb-2">Otros Comprobantes</h4>
                                    <div className="grid grid-cols-3 gap-2">
                                        {ungroupedReceipts.map((r: any, i: number) => (
                                            <button
                                                ...
                                                className="block w-full aspect-square bg-gray-300 rounded overflow-hidden relative border hover:border-blue-500 hover:shadow-lg transition-all cursor-pointer"
                                            >
                                                <img src={r.url} alt={`Receipt ${i + 1}`} className="w-full h-full object-cover" />
                                            </button>
                                        ))}
                                    </div>
                                </div>
                 ...
                 <div className="fixed inset-0 bg-black bg-opacity-90 z-50 flex flex-col items-center justify-center p-4">
                    <div className="relative w-full max-w-md bg-white rounded-lg overflow-hidden flex flex-col max-h-[90vh]">
                        <div className="p-2 bg-gray-100 font-bold text-center">
                            Revisar Foto - {currentGastoType === 'combustible' ? 'Combustible' : 
                                            currentGastoType === 'hielo' ? 'Hielo' :
                                            currentGastoType === 'aguaBebidas' ? 'Agua/Bebidas' :
                                            currentGastoType === 'gastoVarios' ? 'Gasto Varios' : 'General'}
                        </div>
            ...
            <div className="opacity-90">
                <h3 className="font-bold text-lg mb-2 text-green-800">Pagos (Solo Admin)</h3>
                <div className="grid grid-cols-2 gap-3 mb-3">
                    {/* New Fields */}
                    <div>
                        <label className="block text-sm font-bold text-gray-700">Precio Acordado</label>
                        <input type="number" name="precioAcordado" value={formData.precioAcordado} onChange={handleChange} disabled={!canEdit('precioAcordado')} className="w-full border p-2 rounded bg-gray-50 disabled:bg-gray-200" />
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-gray-700">Horas Acordadas</label>
                        <input type="number" name="horasAcordadas" value={formData.horasAcordadas} onChange={handleChange} disabled={!canEdit('horasAcordadas')} className="w-full border p-2 rounded bg-gray-50 disabled:bg-gray-200" />
                    </div>
                </div>
...
interface CameraButtonsProps {
    ...
    handleFileSelect: (e: React.ChangeEvent<HTMLInputElement>, gastoType: string) => void;
    uploading: boolean;
}

const CameraButtons: React.FC<CameraButtonsProps> = ({ gastoType, startCamera, handleFileSelect, uploading }) => (
    <>
